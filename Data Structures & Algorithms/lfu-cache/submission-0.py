...
class LFUCache:
    ...
    def incrementUses(self, node: 'ListNode') -> None:
        ...
        if self.lists[node.uses].isEmpty() and node.uses == self.min_uses:
            print(node.key, node.val, node.uses)
            print('BEFORE:', self.min_uses)
            self.updateMinUses()
            print('AFTER:', self.min_uses)
            print()
