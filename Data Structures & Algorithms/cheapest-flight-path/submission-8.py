...
class Solution:
    def findCheapestPrice(self, n: int, flights: List[List[int]], src: int, dst: int, k: int) -> int:
        ...
        def queue_optimized_bellman_ford(n, flights, src, dst, k):
            graph = defaultdict(list)
            for u, v, p in flights:
                graph[u].append((v, p))
            ...
            queue = deque([(src, 0, 0)])
            while len(queue) > 0:
                u, uc, us = queue.popleft()
                if us > k:
                    continue
                for v, vw in graph[u]:
                    if uc + vw < dist[v]:
                        dist[v] = uc + vw
                        queue.append((v, dist[v], us+1))
    
            if dist[dst] == sys.maxsize:
                return -1
            return dist[dst]

        return queue_optimized_bellman_ford(n, flights, src, dst, k)